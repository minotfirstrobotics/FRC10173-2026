...
class SS_Kraken(commands2.Subsystem):
    def __init__(self, device_id: int, canbus: CANBus, dashboard_name: str, 
                 ...
                 p: float=0.01, i: float=0.0, d: float=0.0, ff: float=0.0, s: float=0.0) -> None:
        super().__init__()
        ...
        self._apply_pidf_to_config(p, i, d, ff, s)
        ...
        wpilib.SmartDashboard.putNumber(f"PIDF/{dashboard_name} P", self.P)
        wpilib.SmartDashboard.putNumber(f"PIDF/{dashboard_name} I", self.I)
        wpilib.SmartDashboard.putNumber(f"PIDF/{dashboard_name} D", self.D)
        wpilib.SmartDashboard.putNumber(f"PIDF/{dashboard_name} FF", self.FF)
        wpilib.SmartDashboard.putNumber(f"PIDF/{dashboard_name} S", self.S)
    ...
    def _apply_pidf_to_config(self, dashboard_p, dashboard_i, dashboard_d, dashboard_ff, dashboard_s):
        self.P = dashboard_p
        self.I = dashboard_i
        self.D = dashboard_d
    ...
    def periodic(self):
        ...
        dashboard_p =  wpilib.SmartDashboard.getNumber(f"PIDF/{self.dashboard_name} P", self.P)
        dashboard_i =  wpilib.SmartDashboard.getNumber(f"PIDF/{self.dashboard_name} I", self.I)
        dashboard_d =  wpilib.SmartDashboard.getNumber(f"PIDF/{self.dashboard_name} D", self.D)
        dashboard_ff = wpilib.SmartDashboard.getNumber(f"PIDF/{self.dashboard_name} FF", self.FF)
        dashboard_s =  wpilib.SmartDashboard.getNumber(f"PIDF/{self.dashboard_name} S", self.S)
        if (dashboard_p != self.P or dashboard_i != self.I or dashboard_d != self.D or dashboard_ff != self.FF or dashboard_s != self.S):
