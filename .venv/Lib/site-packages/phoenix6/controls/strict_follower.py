...
class StrictFollower:
    """
    Follow the motor output of another Talon while ignoring the leader's invert
    setting.
    
    The follower will atomically change its output type when it receives the
    leader's latest output status signal (DutyCycle, MotorVoltage, TorqueCurrent).
    If Talon is in torque control, the torque is copied - which will increase the
    total torque applied. If Talon is in duty cycle output control, the duty cycle
    is matched. If Talon is in voltage output control, the motor voltage is matched.
    Motor direction is strictly determined by the configured invert and not the
    leader. If you want motor direction to match or oppose the leader, use Follower
    instead.
    
    The leader must ensure the status signal corresponding to its control output
    type (DutyCycle, MotorVoltage, TorqueCurrent) is enabled. The update rate of the
    status signal determines the update rate of the follower's output and should be
    no slower than 20 Hz.
    
    :param leader_id: Device ID of the leader to follow.
    :type leader_id: int
    """
    ...
    def _send_request(self, network: str, device_hash: int) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlStrictFollower(ctypes.c_char_p(bytes(network, 'utf-8')), device_hash, self.update_freq_hz, self.leader_id))

    
    def with_leader_id(self, new_leader_id: int) -> 'StrictFollower':
        """
